...
## NoSQL
NoSQL 是**键-值数据库**、**文档型数据库**、**列型数据库**或**图数据库**的统称。数据库是非规范化的，表联结大多在应用程序代码中完成。大多数 NoSQL 无法实现真正符合 ACID 的事务，支持[最终一致](https://github.com/Xianhuii/system-design-primer/blob/master/README-zh-Hans.md#%E6%9C%80%E7%BB%88%E4%B8%80%E8%87%B4%E6%80%A7)。

**BASE** 通常被用于描述 NoSQL 数据库的特性。相比 [CAP 理论](https://github.com/Xianhuii/system-design-primer/blob/master/README-zh-Hans.md#cap-%E7%90%86%E8%AE%BA)，BASE 强调可用性超过一致性。
- **基本可用** - 系统保证可用性。
...
### 列型存储
![[Pasted image 20250114195114.png]]
...
### 图数据库
![[Pasted image 20250114195624.png]]
抽象模型： 图

在图数据库中，一个节点对应一条记录，一个弧对应两个节点之间的关系。图数据库被优化用于表示外键繁多的复杂关系或多对多关系。

图数据库为存储复杂关系的数据模型，如社交网络，提供了很高的性能。它们相对较新，尚未广泛应用，查找开发工具或者资源相对较难。许多图只能通过 [REST API](https://github.com/Xianhuii/system-design-primer/blob/master/README-zh-Hans.md#%E8%A1%A8%E8%BF%B0%E6%80%A7%E7%8A%B6%E6%80%81%E8%BD%AC%E7%A7%BBrest) 访问。
...
## SQL还是NoSQL
![[Pasted image 20250114195818.png]]
选取 **SQL** 的原因:
- 结构化数据
- 严格的模式
- 关系型数据
...
# 缓存
![[Pasted image 20250114200552.png]]
...
## 客户端缓存
缓存可以位于客户端（操作系统或者浏览器），[服务端](https://github.com/Xianhuii/system-design-primer/blob/master/README-zh-Hans.md#%E5%8F%8D%E5%90%91%E4%BB%A3%E7%90%86web-%E6%9C%8D%E5%8A%A1%E5%99%A8)或者不同的缓存层。

## CDN缓存
[CDN](https://github.com/Xianhuii/system-design-primer/blob/master/README-zh-Hans.md#%E5%86%85%E5%AE%B9%E5%88%86%E5%8F%91%E7%BD%91%E7%BB%9Ccdn) 也被视为一种缓存。

## Web服务器缓存
[反向代理](https://github.com/Xianhuii/system-design-primer/blob/master/README-zh-Hans.md#%E5%8F%8D%E5%90%91%E4%BB%A3%E7%90%86web-%E6%9C%8D%E5%8A%A1%E5%99%A8)和缓存（比如 [Varnish](https://www.varnish-cache.org/)）可以直接提供静态和动态内容。Web 服务器同样也可以缓存请求，返回相应结果而不必连接应用服务器。
...
### 缓存模式
![[Pasted image 20250114201129.png]]
...
### 直写模式
![[Pasted image 20250114201858.png]]
...
### 回写模式
![[Pasted image 20250114202029.png]]
在回写模式中，应用执行以下操作：
- 在缓存中增加或者更新条目
- 异步写入数据，提高写入性能。
#### 回写模式的缺点：
...
- 执行直写模式比缓存或者回写模式更复杂。
### 刷新
![[Pasted image 20250114202119.png]]
...
# 异步
![[Pasted image 20250114202358.png]]
...
# 通讯
![[Pasted image 20250114203612.png]]
## 超文本传输协议（HTTP）
...
## 传输控制协议（TCP）
![[Pasted image 20250114203743.png]]
...
为了确保高吞吐量，Web 服务器可以保持大量的 TCP 连接，从而导致高内存使用。在 Web 服务器线程间拥有大量开放连接可能开销巨大，消耗资源过多，也就是说，一个 [memcached](https://github.com/Xianhuii/system-design-primer/blob/master/README-zh-Hans.md#memcached) 服务器。[连接池](https://en.wikipedia.org/wiki/Connection_pool) 可以帮助除了在适用的情况下切换到 UDP。
...
- 你想对网络吞吐量自动进行最佳评估。
## 用户数据包协议（UDP）
![[Pasted image 20250114204544.png]]
...
## 远程控制调用协议（RPC）
![[Pasted image 20250114204652.png]]
