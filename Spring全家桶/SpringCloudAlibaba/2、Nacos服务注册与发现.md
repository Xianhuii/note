...
# 2 理论
## 2.1 数据模型
对于注册中心而言，最终要的数据无非是服务的`name`和`IP地址`。

随着功能的增强，它还需要保存对应的数据信息：
- 实例健康状态
- 权重
- ……

Nacos使用`服务-集群-实例`的三层数据模型

其他注册中心的数据模型：
- Zookeeper：没有针对服务发现设置数据模型，它的数据是以一种更加抽象的树形 K-V 组织的，因 此理论上可以存储任何语义的数据。
- Eureka/Consul：支持实例级别的数据扩展。
